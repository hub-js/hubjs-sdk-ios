...
namespace :test do
  ...
  desc 'Run the HubjsTracker Unit tests'
  task ios: :prepare do
    run_tests('HubjsTracker', 'iphonesimulator', 'platform=iOS Simulator,name=iPhone 6,OS=12.4')
    build_failed('tests') unless $?.success?
    run_tests('HubjsTracker', 'iphonesimulator', 'platform=iOS Simulator,name=iPhone 6,OS=11.4')
    build_failed('tests') unless $?.success?
    # When running Quick on iOS 10.3.1 it fails with a EXC_BAD_ACCESS
    # run_tests('HubjsTracker', 'iphonesimulator', 'platform=iOS Simulator,name=iPhone 6,OS=10.3.1')
    # build_failed('tests') unless $?.success?
  end

  desc 'Build the HubjsTracker iOS demo'
  task ios_demo: :prepare do
    run_build('ios', 'iphonesimulator')
    build_failed('iOS') unless $?.success?
  end

  desc 'Build the HubjsTracker OSX demo'
  task osx_demo: :prepare do
    run_build('macos', 'macosx', 'platform=macOS,arch=x86_64')
    build_failed('macOS') unless $?.success?
  end

  desc 'Build the HubjsTracker tvOS demo'
...
desc 'Run the HubjsTracker tests for iOS & Mac OS X'
task :test do
...
desc 'Check the integration of HubjsTracker with package managers'
...
def run_build(scheme, sdk, destination = 'platform=iOS Simulator,name=iPhone 6,OS=10.3.1')
  sh("xcodebuild -workspace HubjsTracker.xcworkspace -scheme '#{scheme}' -sdk '#{sdk}' -destination '#{destination}' -configuration Release clean build | xcpretty -c ; exit ${PIPESTATUS[0]}") rescue nil
end

def run_tests(scheme, sdk, destination = 'platform=iOS Simulator,name=iPhone 6,OS=10.3.1')
  sh("xcodebuild -workspace HubjsTracker.xcworkspace -scheme '#{scheme}' -sdk '#{sdk}' -destination '#{destination}' -configuration Debug clean test | xcpretty -c ; exit ${PIPESTATUS[0]}") rescue nil
end
