...
class TrackerSpec: QuickSpec {
    override func spec() {
        ...
        describe("init") {
            it("should be able to initialized the HubjsTracker with a URL ending on `tracking.php`") {
                let tracker = HubjsTracker(siteId: "5", baseURL: URL(string: "https://analytics.hub-js.com/tracking.php")!)
                expect(tracker).toNot(beNil())
            }
            it("should be released if no external strong reference exists (no retain cycles") {
                weak var tracker = HubjsTracker(siteId: "5", baseURL: URL(string: "https://analytics.hub-js.com/tracking.php")!)
        ...
        describe("forcedVisitorID") {
            ...
            it("it doesn't change the existing value if set to an invalid one") {
                let tracker = HubjsTracker.init(siteId: "spec", baseURL: URL(string: "http://analytics.hub-js.com/tracking.php")!)
            ...
            it("should persist and restore the value") {
                let tracker = HubjsTracker.init(siteId: "spec", baseURL: URL(string: "http://analytics.hub-js.com/tracking.php")!)
                tracker.forcedVisitorId = "0123456789abcdef"
                let tracker2 = HubjsTracker.init(siteId: "spec", baseURL: URL(string: "http://analytics.hub-js.com/tracking.php")!)
