...
/// The Hubjs Tracker is a Swift framework to send analytics to the Hubjs server.
///
/// ## Basic Usage
...
final public class HubjsTracker: NSObject {
    ...
    @objc public var isOptedOut: Bool {
        get {
            return hubjsUserDefaults.optOut
        }
        set {
            hubjsUserDefaults.optOut = newValue
        }
    }
    ...
    @objc public var userId: String? {
        get {
            return hubjsUserDefaults.visitorUserId
        }
        set {
            hubjsUserDefaults.visitorUserId = newValue
            visitor = Visitor.current(in: hubjsUserDefaults)
        }
    }
    ...
    @objc public var forcedVisitorId: String? {
        get {
            return hubjsUserDefaults.forcedVisitorId
        }
        set {
            ...
            if let newValue = newValue {
                ...
                if isValidString {
                    hubjsUserDefaults.forcedVisitorId = newValue
                } else {
                ...
                hubjsUserDefaults.forcedVisitorId = nil
            }
            visitor = Visitor.current(in: hubjsUserDefaults)
        }
    }
    
    internal var hubjsUserDefaults: HubjsUserDefaults
    ...
    /// This logger is used to perform logging of all sorts of Hubjs related information.
    ...
    internal static var _sharedInstance: HubjsTracker?
    ...
    required public init(siteId: String, queue: Queue, dispatcher: Dispatcher) {
        ...
        self.hubjsUserDefaults = HubjsUserDefaults(suiteName: "\(siteId)\(dispatcher.baseURL.absoluteString)")
        self.visitor = Visitor.current(in: hubjsUserDefaults)
        self.session = Session.current(in: hubjsUserDefaults)
        super.init()
        startNewSession()
        startDispatchTimer()
    }
    ...
    @objc public func dispatch() {
        guard !isDispatching else {
            logger.verbose("HubjsTracker is already dispatching.")
            return
        }
...
extension HubjsTracker {
    ...
    public func startNewSession() {
        hubjsUserDefaults.previousVisit = hubjsUserDefaults.currentVisit
        hubjsUserDefaults.currentVisit = Date()
        hubjsUserDefaults.totalNumberOfVisits += 1
...
extension HubjsTracker {
    ...
    /// Tracks a screenview.
    ///
    /// This method can be used to track hierarchical screen names, e.g. screen/settings/register. Use this to create a hierarchical and logical grouping of screen views in the Hubjs web interface.
    ///
    ...
    public func trackOrder(id: String, items: [OrderItem], revenue: Float, subTotal: Float? = nil, tax: Float? = nil, shippingCost: Float? = nil, discount: Float? = nil) {
        let lastOrderDate = hubjsUserDefaults.lastOrder
        ...
        hubjsUserDefaults.lastOrder = Date()
    }
}
...
extension HubjsTracker {
    ...
    /// - Parameter index: The index of the dimension. A dimension with this index must be setup in the Hubjs backend.
...
extension HubjsTracker {
    @objc public func copyFromOldSharedInstance() {
        hubjsUserDefaults.copy(from: UserDefaults.standard)
    }
}

extension HubjsTracker {
    /// The version of the Hubjs SDKs
    @objc public static let sdkVersion = "1.0"
}
